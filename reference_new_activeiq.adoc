...
summary: Découvrez les nouvelles fonctionnalités et améliorations de Digital Advisor. 
---
...
La fonctionnalité de machine virtuelle sera supprimée de Digital Advisor en octobre 2026.
...
La durabilité est désormais prise en charge sur les systèmes E-Series et StorageGRID .  Vous pouvez consulter une liste d’actions recommandées et d’indicateurs environnementaux qui affichent des projections en matière d’énergie, d’utilisation directe du carbone et de chaleur à partir du tableau de bord de durabilité pour ces systèmes. Pour en savoir plus, consultezlink:https://docs.netapp.com/us-en/active-iq/sustainability-dashboard-overview.html["Présentation du tableau de bord Sustainability"^] .
...
* Le flux de travail Bien-être inclut désormais le widget Durabilité, qui affiche le nombre d'actions recommandées au niveau du client, de la liste de surveillance, du site et du groupe. Vous pouvez sélectionner le nombre d'actions pour obtenir une vue détaillée de ces actions recommandées sur le tableau de bord Durabilité. Pour en savoir plus, consultez link:https://docs.netapp.com/us-en/active-iq/learn-about-sustainability.html["Analysez la durabilité de vos systèmes de stockage"].
...
* L'option *Sauvegarde sans instantanés* est renommée *Avec copies d'instantanés* avec une modification de sa fonctionnalité. link:https://docs.netapp.com/us-en/active-iq/reference_aiq_faq.html#storage-efficiency["En savoir plus >>"].
...
L'efficacité du stockage SAN et NAS est disponible, au niveau du nœud, pour les systèmes ONTAP, y compris AFF A-Series, AFF C190, All SAN Array et FAS500.
...
=== Digital Advisor s'intègre à la NetApp Console

Active IQ Digital Advisor est renommé Digital Advisor et est désormais intégré à NetApp Console, la console de gestion unifiée de NetApp pour les environnements multicloud hybrides. link:https://docs.netapp.com/us-en/active-iq/digital-advisor-integration-with-console.html["En savoir plus >>"].
...
Les utilisateurs de la NetApp Console peuvent désormais ouvrir les liens Digital Advisor dans de nouveaux onglets, le cas échéant, de la même manière que la fonctionnalité existante dans Digital Advisor.
...
* Dans l'e-mail de bilan bien-être, vous trouverez des informations sur la protection contre les ransomwares au lieu des renouvellements.
...
En sélectionnant la case bleue « Résumé des volumes » dans le tableau SVM, une fenêtre contextuelle affiche des informations détaillées sur les volumes hébergés ou associés au numéro de série ou au nœud physique spécifique.
...
+
** *Augmentation de capacité :* Vous serez averti si l’un des sites GRID dépasse le seuil de 70 % de la capacité existante. Vous avez la possibilité d’ajouter de la capacité pour les StorageGRIDs du site pour les 1, 3 et 6 prochains mois si le seuil de capacité risque de dépasser 70 %.
** *Renouvellements :* Pour tout système StorageGRID dont le contrat de licence a expiré ou arrive à expiration dans les 6 prochains mois, vous serez averti. Vous pouvez sélectionner un ou plusieurs systèmes pour soumettre une demande à l’équipe d’assistance NetApp pour le renouvellement.
...
* *GRID Viewer* : Depuis le widget *Configuration*, vous pouvez consulter la configuration détaillée de la GRID en sélectionnant le lien *GRID Viewer*. Depuis le widget *Configuration*, vous pouvez télécharger les Site Details et Capacity Details pour la StorageGRID sélectionnée en sélectionnant le bouton *Download* dans l’écran *GRID Viewer*.
* *Détails du site :* cet onglet fournit le récapitulatif de la grille et les nœuds de stockage disponibles pour chaque site.
* *Résumé GRID :* Contient des informations de base, telles que le type de licence, la capacité de la licence, le nombre de nœuds installés, la durée du support (date de fin du contrat de licence), le nœud d'administration principal et le site principal du nœud d'administration principal. Cet onglet fournit également le nom du site et le nombre de nœuds de stockage associés au site correspondant. Dans cette version, vous pouvez consulter la liste des noms de nœuds en sélectionnant le lien hypertexte disponible pour afficher les nœuds de stockage du site correspondant.
...
Le bouton Tier Data a été ajouté au tableau de bord FabricPool, et il vous permet de classer les données vers des niveaux de stockage d’objets à faible coût à l’aide de NetApp Console.
...
Vous pouvez utiliser les API de Digital Advisor pour extraire les données qui vous intéressent et les intégrer directement dans le flux de travail de votre entreprise. link:https://docs.netapp.com/us-en/active-iq/concept_overview_API_service.html["En savoir plus >>"].
...
Vous pouvez consulter le taux d'efficacité du stockage et les économies réalisées par votre système de stockage avec et sans copies de snapshot pour les systèmes AFF, les systèmes non-AFF ou les deux. Vous pouvez consulter les informations sur l'efficacité du stockage au niveau d'un nœud. link:https://docs.netapp.com/us-en/active-iq/task_analyze_storage_efficiency.html["Découvrez comment"].
