...
---
= Détectez les risques de sécurité avec Digital Advisor et prenez des mesures correctives.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
. Sélectionnez *Actions* dans le widget *Sécurité et défense contre les ransomwares*. image:Security_Image 2 Ransomware attacks.png["Widget failles de sécurité"]
...
. Sélectionnez l'onglet *Risques uniques*, puis sélectionnez le lien dans la colonne *Mesures correctives*. image:Corrective Action_Image 2 Ransomware attacks.png["Colonne action corrective"]
+
