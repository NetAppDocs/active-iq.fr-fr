...
---
= Découvrez les rapports de Digital Advisor
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
| Résumé analytique de l'évaluation de l'état de santé | Fournit des informations sur divers indicateurs clés de performance (KPI) de Digital Advisor au format PowerPoint (PPT), chaque KPI étant décrit sur une diapositive différente. | Oui | Oui | Non 
...
| Sécurité | Fournit une vue d'ensemble complète et exportable des risques de sécurité du stockage ONTAP et des lacunes de conformité à travers les clusters, les SVM et les volumes. | Oui | Non | Non 
