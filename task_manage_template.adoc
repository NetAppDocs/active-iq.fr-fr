...
---
= Gérer les modèles de dérive de configuration dans Digital Advisor
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
.Étapes
. Dans le volet de gauche, sélectionnez *Config Drift*.
. Sélectionnez image:clone_icon.png["clonage"] pour créer une copie du modèle.
. Sélectionnez image:share_icon.png["partagez"] et saisissez les noms d'utilisateur avec lesquels vous souhaitez partager le modèle.
+

NOTE: Si vous saisissez l'adresse e-mail de l'utilisateur au lieu du nom d'utilisateur, le modèle ne sera pas partagé.

. Sélectionnez image:edit_icon.png["modifier"] pour mettre à jour les détails du modèle.
. Sélectionnez image:delete_icon.png["supprimer"] pour supprimer le modèle.
