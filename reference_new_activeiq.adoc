...
* Le flux de travail Bien-être inclut désormais le widget Durabilité, qui fournit le nombre d'actions recommandées aux niveaux du client, de la liste de surveillance, du site et du groupe.  Vous pouvez cliquer sur le nombre d’actions pour obtenir une vue détaillée de ces actions recommandées sur le tableau de bord Durabilité. Pour en savoir plus, consultezlink:https://docs.netapp.com/us-en/active-iq/learn-about-sustainability.html["Analysez la durabilité de vos systèmes de stockage"] .
...
En cliquant sur la case bleue « Résumé des volumes » dans le tableau SVM, une fenêtre contextuelle affiche des informations détaillées sur les volumes qui sont hébergés ou associés au numéro de série ou au nœud physique spécifique.
