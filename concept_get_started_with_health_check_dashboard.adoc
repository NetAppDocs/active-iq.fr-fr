...
* *AutoSupport Adoption* : Affiche le nombre et le pourcentage de systèmes avec AutoSupport activé. Vous pouvez également consulter les systèmes marqués comme « Refusé », ceux avec *HTTPS* et *AutoSupport on Demand* activés, ainsi que *Perte de signal* pour les systèmes ayant cessé d'envoyer des données AutoSupport au cours des 7 derniers jours. Pour consulter votre score de vérification de l'état et les informations sur les systèmes de votre base client installée, cliquez sur le widget *AutoSupport Adoption*.
